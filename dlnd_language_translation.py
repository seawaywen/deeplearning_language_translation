...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    source_id_list = list(map(lambda x: [source_vocab_to_int[key] for key in x.split()], 
                          source_sentences))
    #print(source_id_list)

    target_sentences = ['{} <EOS>'.format(sentence) for sentence in target_text.split('\n')]
    target_id_list = list(map(lambda x: [target_vocab_to_int[key] for key in x.split()], 
                          target_sentences))
    #print(target_sentences)
    #print(target_id_list)    
                 
    return source_id_list, target_id_list
...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    removed_last_id_data = target_data[..., 0:-1]
    go_id_tensor = tf.fill([batch_size, 1], target_vocab_to_int['<GO>'])
    decoded_input = tf.concat([go_id_tensor, removed_last_id_data], 1)
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    drop_wrapper = tf.contrib.rnn.DropoutWrapper(lstm_cell, output_keep_prob=keep_prob)
    rnn_cells = tf.contrib.rnn.MultiRNNCell([drop_wrapper] * num_layers)
    rnn_output, rnn_state = tf.nn.dynamic_rnn(rnn_cells, rnn_inputs, dtype=tf.float32)
    return rnn_state
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    output, _, _ = tf.contrib.seq2seq.dynamic_rnn_decoder(
        dropout_cell, train_decoder_fn, 
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    decoder_infe_fn = tf.contrib.seq2seq.simple_decoder_fn_inference(
...
def decoding_layer(dec_embed_input, dec_embeddings, encoder_state, vocab_size, sequence_length, rnn_size,
    ...
    dropout_cell = tf.contrib.rnn.DropoutWrapper(rnn_cell, keep_prob)
    ...
    with tf.variable_scope('decoding') as decoding_scope:
        ...
        training_logits = decoding_layer_train(
            encoder_state, dropout_cell, dec_embed_input, 
    ...
    with tf.variable_scope('decoding', reuse=True) as decoding_scope:
        infe_logits = decoding_layer_infer(
            encoder_state, dropout_cell, 
            dec_embeddings, start_sequence_id, end_sequence_id, 
            sequence_length, vocab_size, decoding_scope, 
            output_fn, keep_prob)
        
    return training_logits, infe_logits
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    :return: Tuple of (Training Logits, Inference Logits)
    """
    embed_encode_input = tf.contrib.layers.embed_sequence(
        input_data, source_vocab_size, enc_embedding_size)
    encode_state = encoding_layer(embed_encode_input, rnn_size, num_layers, keep_prob)
    decode_input = process_decoding_input(target_data, target_vocab_to_int, batch_size)
    ...
    train_logits, inference_logits = decoding_layer(
        dec_embed_input, dec_embeddings, 
        encode_state, target_vocab_size, sequence_length, 
...
def sentence_to_seq(sentence, vocab_to_int):
    ...
    word_id_list = [vocab_to_int.get(i, vocab_to_int['<UNK>']) for i in lower_sentence.split()]
    return word_id_list
...
#translate_sentence = 'he is yellow .'
